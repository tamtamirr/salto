...
describe('deploy command', () => {
  ...
  describe('invalid deploy', () => {
    it('should fail gracefully', async () => {
      workspace.errors.mockResolvedValue(mocks.mockErrors([{ severity: 'Error', message: 'some error', detailedMessage: 'some error' }]))
      const result = await action({
        ...cliCommandArgs,
        input: {
          force: false,
    ...
    it('should allow the user to cancel when there are warnings', async () => {
      workspace.errors.mockResolvedValue(mocks.mockErrors([{ severity: 'Warning', message: 'some warning', detailedMessage: 'some warning' }]))
  ...
  describe('when deploy result makes the workspace invalid', () => {
    beforeEach(() => {
      workspace.updateNaclFiles.mockImplementationOnce(async () => {
        ...
        workspace.errors.mockResolvedValueOnce(mocks.mockErrors([{ severity: 'Error', message: '', detailedMessage: '' }]))
