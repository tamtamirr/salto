...
describe('workspace', () => {
  ...
  describe('updateWorkspace', () => {
    ...
    it('with validation errors', async () => {
      mockWsFunctions.errors.mockResolvedValueOnce(mockErrors([{ message: 'Error BLA', severity: 'Error', detailedMessage: 'Error BLA' }]))
      const result = await updateWorkspace({
        workspace: mockWs,
        output: cliOutput,
        changes: dummyChanges.map(change => ({ change, serviceChanges: [change] })),
  ...
  describe('applyChangesToWorkspace', () => {
    ...
    it('should return false on error', async () => {
      mockWsFunctions.errors.mockResolvedValue(mockErrors([{ message: 'Error BLA', severity: 'Error', detailedMessage: 'Error BLA' }]))
      const res = await applyChangesToWorkspace({
        workspace: mockWs,
        changes,
        mode: 'default',
