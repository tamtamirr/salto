...
const FieldDescriptions = {
  metadata:
    'Fetch only referenced records to streamline metadata retrieval, enhancing efficiency by limiting the scope to essential elements',
  data: 'Fetch only referenced records to streamline data retrieval, enhancing efficiency by limiting the scope to essential elements',
  dataAllowReferenceTo:
    'Fetch only referenced records to streamline data retrieval, enhancing efficiency by limiting the scope to essential elements',
}
...
export const warningSettingsType = new ObjectType({
...
export const dataManagementType = new ObjectType({
...
export const clientPollingConfigType = new ObjectType({
...
export const clientDeployConfigType = new ObjectType({
...
export const clientRateLimitConfigType = new ObjectType({
...
export const clientRetryConfigType = new ObjectType({
...
export const readMetadataChunkSizeConfigType =
...
export const metadataConfigType = createMatchingObjectType<MetadataParams>({
...
const optionalFeaturesType = createMatchingObjectType<OptionalFeatures>({
  ...
  fields: {
    ...
    enumFieldPermissions: { refType: BuiltinTypes.BOOLEAN },
  },
  annotations: {
    [CORE_ANNOTATIONS.ADDITIONAL_PROPERTIES]: false,
  },
...
export const changeValidatorConfigType =
...
const defaultGeneralSettings: GeneralFetchParameters = {
  ...
  maxInstancesPerType: 2500,
  warningSettings: {
    nonQueryableFields: false,
  },
}

export const defaultCPQConfig: FetchParameters = {
  metadata: {
    include: [
      { metadataType: '.*', namespace: 'sbaa', name: '.*' },
      { metadataType: '.*', namespace: 'SBQQ', name: '.*' },
    ],
  },
  data: {
    includeObjects: [
    ...
    saltoIDSettings: {
      defaultIdFields: [constants.CUSTOM_OBJECT_ID_FIELD],
    },
    brokenOutgoingReferencesSettings: {
...
const fetchParametersFieldDefinitions = {
  metadata: {
    refType: metadataConfigType,
    annotations: { [CORE_ANNOTATIONS.DESCRIPTION]: FieldDescriptions.metadata },
  },
  data: {
    refType: dataManagementType,
    annotations: { [CORE_ANNOTATIONS.DESCRIPTION]: FieldDescriptions.data },
  },
}

const fetchParametersObjectType = createMatchingObjectType<FetchParameters>({
  elemID: new ElemID(constants.SALESFORCE, 'fetchParameters'),
  fields: fetchParametersFieldDefinitions,
})

const generalSettingsType = createMatchingObjectType<GeneralFetchParameters>({
  elemID: new ElemID(constants.SALESFORCE, 'generalFetchParameters'),
  fields: {
    ...fetchParametersFieldDefinitions,
    ...
    additionalImportantValues: { refType: new ListType(importantValueType) },
  },
})
...
export const configType = createMatchingObjectType<SalesforceConfig>({
  ...
  fields: {
    generalSettings: {
      refType: generalSettingsType,
      annotations: {
        _required: true,
        [CORE_ANNOTATIONS.DEFAULT]: defaultGeneralSettings,
      },
    },
    [CLIENT_CONFIG]: { refType: clientConfigType },
    [DEPLOY_CONFIG]: {
    ...
    CPQ: {
      refType: fetchParametersObjectType,
      annotations: {
        [CORE_ANNOTATIONS.ALIAS]: CPQ_MANAGED_PACKAGE,
        [CORE_ANNOTATIONS.DEFAULT]: defaultCPQConfig,
      },
    },
  },
  annotations: {
